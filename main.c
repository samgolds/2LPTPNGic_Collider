...
// *** Collider Addition (End) ***
  ...
  printf("  Klong_max = %d  Spin = %d    Nu = %+.2e    Phase = %+.2e\n", Klong_max, Spin, Nu, Phase);
...
void displacement_fields(void){
  ...
  // *** Collider Addition (Start) ***
    ...
    // invariant
    double complex kmag_1p5_plus_inu; 
    double complex kmag_1p5_min_inu;

    // Define phase coefficients e^{i delta}
    double complex exp_plus_iphase;
    double complex exp_minus_iphase;

    // Temporary complex number for manipulations
    double complex temp_complex_double;

    // Define the k^{3/2+i\nu}phi_G(k) field and its Fourier transform, as well
    // as the k^{3/2-i\nu}phi_G(k) field.
    fftw_complex *(ck_1p5_plus_inu_phi); // THESE SHOULD NOT BE RFFTW
    fftw_real *(k_1p5_plus_inu_phi);     // THIS SHOULD NOT BE A REAL FIELD

    fftw_complex *(ck_1p5_min_inu_phi);  // THIS SHOULD NOT BE A REAL FIELD
    fftw_real *(k_1p5_min_inu_phi);     // THIS SHOULD NOT BE A REAL FIELD
  ...
  /*** Notice also that there is a minus to convert from Bardeen to gravitational potential ***/
    ...
    for (i = 0; i < Local_nx; i++)
      for (j = 0; j < Nmesh; j++)
        for (k = 0; k < Nmesh; k++)
        {
          ...
          #ifdef OUTPUT_DF
            phi_lin[coord] = pot[coord];
            coord_lin[coord] = coord; // ((i+Local_x_start) * Nmesh + j) * (2 * (Nmesh / 2 + 1)) + k;
            coord_i[coord] = i+Local_x_start;
            coord_j[coord] = j;
            coord_k[coord] = k;
          #endif
        }
    ...
    #ifdef OUTPUT_DF
	    write_phi_lin_field_data();//write_density_field_data();
    #endif
  ...
  if (ThisTask == 0)
    {
      printf("\nComputing oscillatory bispectra non-Gaussian potential... ");
      fflush(stdout);
    };

    // Initialize all FFT's for auxiliary field
    ck_1p5_plus_inu_phi = (fftw_complex *)malloc(bytes = sizeof(fftw_real) * TotalSizePlusAdditional);
    k_1p5_plus_inu_phi = (fftw_real *)ck_1p5_plus_inu_phi;
    ASSERT_ALLOC(ck_1p5_plus_inu_phi);

    ck_1p5_min_inu_phi = (fftw_complex *)malloc(bytes = sizeof(fftw_real) * TotalSizePlusAdditional);
    k_1p5_min_inu_phi = (fftw_real *)ck_1p5_min_inu_phi;
    ASSERT_ALLOC(ck_1p5_min_inu_phi);
    ...
    for (i = 0; i < Local_nx; i++)
      for (j = 0; j < Nmesh; j++)
        ...
        {
          coord = (i * Nmesh + j) * (Nmesh / 2 + 1) + k;
          ck_1p5_plus_inu_phi[coord].re = 0.0;
          ck_1p5_plus_inu_phi[coord].im = 0.0;
          ck_1p5_min_inu_phi[coord].re = 0.0;
          ck_1p5_min_inu_phi[coord].im = 0.0;
          cpsi[coord].re = 0.0;
          cpsi[coord].im = 0.0;
          cpot_sq[coord].re = 0.0;
          cpot_sq[coord].im = 0.0;
    ...
    if (ThisTask == 0)
    {
      printf("TEST POINT TWO");
      fflush(stdout);
    };

    // Multiply by k^3/2+/-i nu
    for (ii = 0; ii < Local_nx; ii++)
      for (j = 0; j < Nmesh; j++)
        for (k = 0; k <= Nmesh / 2; k++)
        {
          ...
          kmag_1p5_plus_inu = cpow(kmag, 3./2.+Nu*I); // For non-scale-invariant replace with  1./2.*(4.-PrimordialIndex)
          kmag_1p5_min_inu  = cpow(kmag, 3./2.-Nu*I); 
          
          ck_1p5_plus_inu_phi[coord].re = creal(kmag_1p5_plus_inu)*cpot[coord].re-cimag(kmag_1p5_plus_inu)*cpot[coord].im;
          ck_1p5_plus_inu_phi[coord].im = creal(kmag_1p5_plus_inu)*cpot[coord].im+cimag(kmag_1p5_plus_inu)*cpot[coord].re;

          ck_1p5_min_inu_phi[coord].re = creal(kmag_1p5_min_inu)*cpot[coord].re-cimag(kmag_1p5_min_inu)*cpot[coord].im;
          ck_1p5_min_inu_phi[coord].im = creal(kmag_1p5_min_inu)*cpot[coord].im+cimag(kmag_1p5_min_inu)*cpot[coord].re;
            ...
            //   if (ii==0){
            //     if(j==1){
            //       printf("A = %.2e   B = %.2e   C = %.2e\n", creal(kmag_1p5_plus_inu), cimag(kmag_1p5_plus_inu), ck_1p5_min_inu_phi[coord].re);
            //     }
            //   }
            // };
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    rfftwnd_mpi(Inverse_plan, 1, pot, Workspace, FFTW_NORMAL_ORDER);
    rfftwnd_mpi(Inverse_plan, 1, k_1p5_plus_inu_phi, Workspace, FFTW_NORMAL_ORDER);
    rfftwnd_mpi(Inverse_plan, 1, k_1p5_min_inu_phi, Workspace, FFTW_NORMAL_ORDER);
...
void initialize_ffts(void)
{
  ...
  ASSERT_ALLOC(Workspace)
    ...
    // Real space FFTs for linear field
    phi_lin = malloc(sizeof(float)*Local_nx*Nmesh*Nmesh);
    coord_lin = malloc(sizeof(long long)*Local_nx*Nmesh*Nmesh);

    // FUCK FUCK FUCK
    coord_i = malloc(sizeof(int)*Local_nx*Nmesh*Nmesh);
    coord_j = malloc(sizeof(int)*Local_nx*Nmesh*Nmesh);
    coord_k = malloc(sizeof(int)*Local_nx*Nmesh*Nmesh);

    if (coord_DF && amplitudes && phases && phi_lin && coord_lin && coord_i && coord_j && coord_k)
      if (ThisTask==0)
